...
_db = get_database()  # Initialize singleton on startup
...
# Research Lab Router (S2)
try:
    from modules.trading_capsule.research.research_routes import router as research_lab_router
    app.include_router(research_lab_router, prefix="/api/research")
    print("[Routes] Research Lab router registered (S2.1-S2.4)")
except ImportError as e:
    print(f"[Routes] Research Lab router not available: {e}")

# Walk Forward Router (S2.6)
try:
    from modules.trading_capsule.research.walkforward.walkforward_routes import router as walkforward_router
    app.include_router(walkforward_router, prefix="/api/research/walkforward")
    print("[Routes] Walk Forward router registered (S2.6)")
except ImportError as e:
    print(f"[Routes] Walk Forward router not available: {e}")

# Capital Allocation Router (S3)
try:
    from modules.trading_capsule.allocation.allocation_routes import router as allocation_router
    app.include_router(allocation_router, prefix="/api/allocation")
    print("[Routes] Capital Allocation router registered (S3)")
except ImportError as e:
    print(f"[Routes] Capital Allocation router not available: {e}")

# Research Reports Router (S2.5)
try:
    from modules.trading_capsule.research.reporting.report_routes import router as report_router
    app.include_router(report_router, prefix="/api/research")
    print("[Routes] Research Reports router registered (S2.5)")
except ImportError as e:
    print(f"[Routes] Research Reports router not available: {e}")

# Portfolio Simulation Router (S4.1)
try:
    from modules.trading_capsule.portfolio.portfolio_routes import router as portfolio_sim_router
    app.include_router(portfolio_sim_router)
    print("[Routes] Portfolio Simulation router registered (S4.1)")
except ImportError as e:
    print(f"[Routes] Portfolio Simulation router not available: {e}")

# Monte Carlo Stress Engine Router (S5)
try:
    from modules.trading_capsule.montecarlo.mc_routes import router as mc_router
    app.include_router(mc_router)
    print("[Routes] Monte Carlo router registered (S5)")
except ImportError as e:
    print(f"[Routes] Monte Carlo router not available: {e}")

# Strategy Profiles Router (STR1)
try:
    from modules.trading_capsule.strategy_profiles.profile_routes import router as profiles_router
    app.include_router(profiles_router)
    print("[Routes] Strategy Profiles router registered (STR1)")
except ImportError as e:
    print(f"[Routes] Strategy Profiles router not available: {e}")

# Strategy Config Router (STR2)
try:
    from modules.trading_capsule.strategy_config.config_routes import router as config_router
    app.include_router(config_router)
    print("[Routes] Strategy Config router registered (STR2)")
except ImportError as e:
    print(f"[Routes] Strategy Config router not available: {e}")

# Strategy Switch Router (STR3)
try:
    from modules.trading_capsule.strategy_switch.switch_routes import router as switch_router
    app.include_router(switch_router)
    print("[Routes] Strategy Switch router registered (STR3)")
except ImportError as e:
    print(f"[Routes] Strategy Switch router not available: {e}")

# Strategy Diagnostics Router (STR4)
try:
    from modules.trading_capsule.strategy_diagnostics.diagnostics_routes import router as diagnostics_router
    app.include_router(diagnostics_router)
    print("[Routes] Strategy Diagnostics router registered (STR4)")
except ImportError as e:
    print(f"[Routes] Strategy Diagnostics router not available: {e}")

# ============================================
# Trading Terminal Routers (TR1-TR3)
# ============================================

# Account Manager Router (TR1)
try:
    from modules.trading_terminal.accounts.account_routes import router as accounts_router
    app.include_router(accounts_router)
    print("[Routes] Account Manager router registered (TR1)")
except ImportError as e:
    print(f"[Routes] Account Manager router not available: {e}")

# Portfolio Monitor Router (TR2)
try:
    from modules.trading_terminal.portfolio.portfolio_routes import router as portfolio_router
    app.include_router(portfolio_router)
    print("[Routes] Portfolio Monitor router registered (TR2)")
except ImportError as e:
    print(f"[Routes] Portfolio Monitor router not available: {e}")

# Trade Monitor Router (TR3)
try:
    from modules.trading_terminal.trades.trade_routes import router as trades_router
    app.include_router(trades_router)
    print("[Routes] Trade Monitor router registered (TR3)")
except ImportError as e:
    print(f"[Routes] Trade Monitor router not available: {e}")

# Risk Dashboard Router (TR4)
try:
    from modules.trading_terminal.risk.risk_routes import router as risk_router
    app.include_router(risk_router)
    print("[Routes] Risk Dashboard router registered (TR4)")
except ImportError as e:
    print(f"[Routes] Risk Dashboard router not available: {e}")

# Strategy Control Router (TR5)
try:
    from modules.trading_terminal.strategy_control.control_routes import router as control_router
    app.include_router(control_router)
    print("[Routes] Strategy Control router registered (TR5)")
except ImportError as e:
    print(f"[Routes] Strategy Control router not available: {e}")

# Trading Dashboard Router (TR6)
try:
    from modules.trading_terminal.dashboard.dashboard_routes import router as dashboard_router
    app.include_router(dashboard_router)
    print("[Routes] Trading Dashboard router registered (TR6)")
except ImportError as e:
    print(f"[Routes] Trading Dashboard router not available: {e}")

# Execution Safety Router (SEC1)
try:
    from modules.trading_capsule.security.execution_safety.safety_routes import router as safety_router
    app.include_router(safety_router)
    print("[Routes] Execution Safety router registered (SEC1)")
except ImportError as e:
    print(f"[Routes] Execution Safety router not available: {e}")

# Strategy Taxonomy Router (STG1)
try:
    from modules.trading_capsule.strategies.strategy_routes import router as strategies_router
    app.include_router(strategies_router)
    print("[Routes] Strategy Taxonomy router registered (STG1)")
except ImportError as e:
    print(f"[Routes] Strategy Taxonomy router not available: {e}")

# Strategy Logic Engine Router (STG2)
try:
    from modules.trading_capsule.strategies.logic.logic_routes import router as logic_router
    app.include_router(logic_router)
    print("[Routes] Strategy Logic Engine router registered (STG2)")
except ImportError as e:
    print(f"[Routes] Strategy Logic Engine router not available: {e}")

# Strategy Statistics Router (STG3)
try:
    from modules.trading_capsule.strategies.statistics.statistics_routes import router as stats_router
    app.include_router(stats_router)
    print("[Routes] Strategy Statistics router registered (STG3)")
except ImportError as e:
    print(f"[Routes] Strategy Statistics router not available: {e}")

# Strategy Diagnostics Router (STG4)
try:
    from modules.trading_capsule.strategies.diagnostics.behavior_routes import router as diagnostics_router
    app.include_router(diagnostics_router)
    print("[Routes] Strategy Behavior Diagnostics router registered (STG4)")
except ImportError as e:
    print(f"[Routes] Strategy Behavior Diagnostics router not available: {e}")

# Strategy Selection Router (STG5)
try:
    from modules.trading_capsule.strategies.selection.selection_routes import router as selection_router
    app.include_router(selection_router)
    print("[Routes] Strategy Selection router registered (STG5)")
except ImportError as e:
    print(f"[Routes] Strategy Selection router not available: {e}")

# API Key Vault Router (SEC2)
try:
    from modules.trading_capsule.security.vault.vault_routes import router as vault_router
    app.include_router(vault_router)
    print("[Routes] API Key Vault router registered (SEC2)")
except ImportError as e:
    print(f"[Routes] API Key Vault router not available: {e}")

# State Reconciliation Router (SECX) — replaced by PHASE 4.2 Execution Reconciliation
# try:
#     from modules.trading_terminal.reconciliation.recon_routes import router as recon_router
#     app.include_router(recon_router)
#     print("[Routes] State Reconciliation router registered (SECX)")
# except ImportError as e:
#     print(f"[Routes] State Reconciliation router not available: {e}")

# Strategy Regime Engine Router (REG)
try:
    from modules.trading_capsule.regime.regime_routes import router as regime_router
    app.include_router(regime_router)
    print("[Routes] Strategy Regime Engine router registered (REG)")
except ImportError as e:
    print(f"[Routes] Strategy Regime Engine router not available: {e}")


# Event Ledger Router
try:
    from modules.core.event_ledger.ledger_routes import router as ledger_router
    app.include_router(ledger_router)
    print("[Routes] Event Ledger router registered")
except ImportError as e:
    print(f"[Routes] Event Ledger router not available: {e}")

# OPS1 Deep Position Monitor Router
try:
    from modules.trading_terminal.operations.positions.position_routes import router as positions_router
    app.include_router(positions_router)
    print("[Routes] OPS1 Position Monitor router registered")
except ImportError as e:
    print(f"[Routes] OPS1 Position Monitor not available: {e}")

# OPS2 Position Lifecycle Router
try:
    from modules.trading_terminal.operations.lifecycle.lifecycle_routes import router as lifecycle_router
    app.include_router(lifecycle_router)
    print("[Routes] OPS2 Position Lifecycle router registered")
except ImportError as e:
    print(f"[Routes] OPS2 Position Lifecycle not available: {e}")

# OPS3 Trade Forensics Router
try:
    from modules.trading_terminal.operations.forensics.forensics_routes import router as forensics_router
    app.include_router(forensics_router)
    print("[Routes] OPS3 Trade Forensics router registered")
except ImportError as e:
    print(f"[Routes] OPS3 Trade Forensics not available: {e}")

# OPS4 Capital Flow Router
try:
    from modules.trading_terminal.operations.capital.capital_routes import router as capital_router
    app.include_router(capital_router)
    print("[Routes] OPS4 Capital Flow router registered")
except ImportError as e:
    print(f"[Routes] OPS4 Capital Flow not available: {e}")

# SEC3 Connection Safety Router
try:
    from modules.security.connection_safety.connection_routes import router as connection_safety_router
    app.include_router(connection_safety_router)
    print("[Routes] SEC3 Connection Safety router registered")
except ImportError as e:
    print(f"[Routes] SEC3 Connection Safety not available: {e}")

# TR6 Unified Dashboard Router
try:
    from modules.trading_terminal.dashboard.dashboard_routes import router as dashboard_router
    app.include_router(dashboard_router)
    print("[Routes] TR6 Unified Dashboard router registered")
except ImportError as e:
    print(f"[Routes] TR6 Unified Dashboard not available: {e}")

# PHASE 1.1 Strategy Doctrine Router
try:
    from modules.trading_capsule.strategy_doctrine.doctrine_routes import router as doctrine_router
    app.include_router(doctrine_router)
    print("[Routes] PHASE 1.1 Strategy Doctrine router registered")
except ImportError as e:
    print(f"[Routes] Strategy Doctrine not available: {e}")

# PHASE 1.2 Execution Styles Router
try:
    from modules.trading_capsule.execution_styles.execution_style_routes import router as execution_styles_router
    app.include_router(execution_styles_router)
    print("[Routes] PHASE 1.2 Execution Styles router registered")
except ImportError as e:
    print(f"[Routes] Execution Styles not available: {e}")

# PHASE 1.3 Position Management Policy Router
try:
    from modules.trading_capsule.position_management.position_policy_routes import router as position_policy_router
    app.include_router(position_policy_router)
    print("[Routes] PHASE 1.3 Position Management Policy router registered")
except ImportError as e:
    print(f"[Routes] Position Management Policy not available: {e}")

# PHASE 1.4 Recovery Engine Router
try:
    from modules.trading_capsule.recovery_engine.recovery_routes import router as recovery_router
    app.include_router(recovery_router)
    print("[Routes] PHASE 1.4 Recovery Engine router registered")
except ImportError as e:
    print(f"[Routes] Recovery Engine not available: {e}")

# PHASE 2.1 Calibration Matrix Router
try:
    from modules.research.calibration_matrix.calibration_routes import router as calibration_router
    app.include_router(calibration_router)
    print("[Routes] PHASE 2.1 Calibration Matrix router registered")
except ImportError as e:
    print(f"[Routes] Calibration Matrix not available: {e}")

# PHASE 2.2 Failure Map Router
try:
    from modules.research.failure_map.failure_routes import router as failure_map_router
    app.include_router(failure_map_router)
    print("[Routes] PHASE 2.2 Failure Map router registered")
except ImportError as e:
    print(f"[Routes] Failure Map not available: {e}")

# PHASE 2.3 Forward Simulation Router
try:
    from modules.research.forward_simulation.forward_routes import router as forward_router
    app.include_router(forward_router)
    print("[Routes] PHASE 2.3 Forward Simulation router registered")
except ImportError as e:
    print(f"[Routes] Forward Simulation not available: {e}")

# PHASE 2.4 Selection Validation Router
try:
    from modules.research.selection_validation.selection_routes import router as selection_router
    app.include_router(selection_router)
    print("[Routes] PHASE 2.4 Selection Validation router registered")
except ImportError as e:
    print(f"[Routes] Selection Validation not available: {e}")

# PHASE 6.1 Hypothesis Engine Router
try:
    from modules.research.hypothesis_engine.hypothesis_routes import router as hypothesis_router
    app.include_router(hypothesis_router)
    print("[Routes] PHASE 6.1 Hypothesis Engine router registered")
except ImportError as e:
    print(f"[Routes] Hypothesis Engine not available: {e}")

# PHASE 6.2 Scenario Engine Router
try:
    from modules.research.scenario_engine.scenario_routes import router as scenario_router
    app.include_router(scenario_router)
    print("[Routes] PHASE 6.2 Scenario Engine router registered")
except ImportError as e:
    print(f"[Routes] Scenario Engine not available: {e}")

# PHASE 6.3 Monte Carlo Engine Router
try:
    from modules.research.monte_carlo_engine.monte_routes import router as monte_router
    app.include_router(monte_router)
    print("[Routes] PHASE 6.3 Monte Carlo Engine router registered")
except ImportError as e:
    print(f"[Routes] Monte Carlo Engine not available: {e}")

# PHASE 6.4 Edge Discovery Engine Router
try:
    from modules.research.edge_discovery_engine.edge_routes import router as edge_router
    app.include_router(edge_router)
    print("[Routes] PHASE 6.4 Edge Discovery Engine router registered")
except ImportError as e:
    print(f"[Routes] Edge Discovery Engine not available: {e}")

# PHASE 7 Correlation Intelligence Router
try:
    from modules.market_intelligence.correlation_engine.correlation_routes import router as correlation_router
    app.include_router(correlation_router)
    print("[Routes] PHASE 7 Correlation Intelligence router registered")
except ImportError as e:
    print(f"[Routes] Correlation Intelligence not available: {e}")

# PHASE 8 Liquidity Intelligence Router
try:
    from modules.market_intelligence.liquidity_intelligence.liquidity_routes import router as liquidity_router
    app.include_router(liquidity_router)
    print("[Routes] PHASE 8 Liquidity Intelligence router registered")
except ImportError as e:
    print(f"[Routes] Liquidity Intelligence not available: {e}")

# PHASE 9 Market Microstructure Router
try:
    from modules.market_intelligence.market_microstructure.microstructure_routes import router as microstructure_router
    app.include_router(microstructure_router)
    print("[Routes] PHASE 9 Market Microstructure router registered")
except ImportError as e:
    print(f"[Routes] Market Microstructure not available: {e}")

# PHASE 10 Portfolio Construction Router
try:
    from modules.portfolio_intelligence.portfolio_construction.portfolio_routes import router as portfolio_router
    app.include_router(portfolio_router)
    print("[Routes] PHASE 10 Portfolio Construction router registered")
except ImportError as e:
    print(f"[Routes] Portfolio Construction not available: {e}")


# PHASE 11 Adaptive Intelligence Router
try:
    from modules.adaptive_intelligence.adaptive_routes import router as adaptive_router
    app.include_router(adaptive_router)
    print("[Routes] PHASE 11 Adaptive Intelligence router registered")
except ImportError as e:
    print(f"[Routes] Adaptive Intelligence not available: {e}")


# PHASE 12 System Intelligence Router
try:
    from modules.system_intelligence.system_routes import router as system_intelligence_router
    app.include_router(system_intelligence_router)
    print("[Routes] PHASE 12 System Intelligence router registered")
except ImportError as e:
    print(f"[Routes] System Intelligence not available: {e}")


# PHASE 13.1 Alpha Factory Router
try:
...
# PHASE 3.1 Position Intelligence Router
try:
    from modules.trading_capsule.position_intelligence.position_routes import router as position_intel_router
    app.include_router(position_intel_router)
    print("[Routes] PHASE 3.1 Position Intelligence router registered")
except ImportError as e:
    print(f"[Routes] Position Intelligence not available: {e}")

# PHASE 3.2 Advanced Trade Health Engine Router
try:
    from modules.trading_capsule.trade_health.health_routes import router as trade_health_router
    app.include_router(trade_health_router)
    print("[Routes] PHASE 3.2 Advanced Trade Health Engine router registered")
except ImportError as e:
    print(f"[Routes] Trade Health Engine not available: {e}")

# PHASE 3.3 Dynamic Risk Engine Router
try:
    from modules.trading_capsule.dynamic_risk.risk_routes import router as dynamic_risk_router
    app.include_router(dynamic_risk_router)
    print("[Routes] PHASE 3.3 Dynamic Risk Engine router registered")
except ImportError as e:
    print(f"[Routes] Dynamic Risk Engine not available: {e}")

# PHASE 3.4 Capital Optimization Engine Router
try:
    from modules.trading_capsule.capital_optimization.allocation_routes import router as capital_router
    app.include_router(capital_router)
    print("[Routes] PHASE 3.4 Capital Optimization Engine router registered")
except ImportError as e:
    print(f"[Routes] Capital Optimization Engine not available: {e}")

# PHASE 4.1 Order State Engine Router
try:
    from modules.execution.order_state.order_routes import router as order_state_router
    app.include_router(order_state_router)
    print("[Routes] PHASE 4.1 Order State Engine router registered")
except ImportError as e:
    print(f"[Routes] Order State Engine not available: {e}")

# PHASE 4.2 Execution Reconciliation Router
try:
    from modules.execution.reconciliation.reconciliation_routes import router as reconciliation_router
    app.include_router(reconciliation_router)
    print("[Routes] PHASE 4.2 Execution Reconciliation router registered")
except ImportError as e:
    print(f"[Routes] Execution Reconciliation not available: {e}")

# PHASE 3.5.1 Alpha Engine Router
try:
    from modules.trading_capsule.alpha_engine.alpha_routes import router as alpha_engine_router
    app.include_router(alpha_engine_router)
    print("[Routes] PHASE 3.5.1 Alpha Engine router registered")
except ImportError as e:
    print(f"[Routes] Alpha Engine not available: {e}")

# PHASE 3.5.2 Signal Ensemble Engine Router
try:
    from modules.trading_capsule.signal_ensemble.ensemble_routes import router as signal_ensemble_router
    app.include_router(signal_ensemble_router)
    print("[Routes] PHASE 3.5.2 Signal Ensemble router registered")
except ImportError as e:
    print(f"[Routes] Signal Ensemble not available: {e}")

# PHASE 3.5.3 Market Structure Engine Router
try:
    from modules.trading_capsule.market_structure.structure_routes import router as market_structure_router
    app.include_router(market_structure_router)
    print("[Routes] PHASE 3.5.3 Market Structure router registered")
except ImportError as e:
    print(f"[Routes] Market Structure not available: {e}")

# PHASE 3.5.4 Advanced Market Context Router
try:
    from modules.trading_capsule.market_context.context_routes import router as market_context_router
    app.include_router(market_context_router)
    print("[Routes] PHASE 3.5.4 Market Context router registered")
except ImportError as e:
    print(f"[Routes] Market Context not available: {e}")

# PHASE 4.3 Slippage Engine Router
try:
    from modules.execution.slippage.slippage_routes import router as slippage_router
    app.include_router(slippage_router)
    print("[Routes] PHASE 4.3 Slippage Engine router registered")
except ImportError as e:
    print(f"[Routes] Slippage Engine not available: {e}")

# PHASE 4.4 Failover Engine Router
try:
    from modules.execution.failover.failover_routes import router as failover_router
    app.include_router(failover_router)
    print("[Routes] PHASE 4.4 Failover Engine router registered")
except ImportError as e:
...
# ============================================
# WebSocket Endpoint for Dashboard (TR6)
# ============================================
...
async def dashboard_websocket(websocket: WebSocket):
    ...
    try:
        from modules.trading_terminal.dashboard.dashboard_stream import dashboard_stream
        
        client_id = dashboard_stream.register_client(websocket)
        
        # Send initial state
        from modules.trading_terminal.dashboard.dashboard_service import dashboard_service
        initial_state = dashboard_service.get_state_dict()
        ...
        while True:
            ...
            try:
                ...
                if msg.get("action") == "subscribe":
                    dashboard_stream.subscribe(client_id, msg.get("types", []))
                elif msg.get("action") == "unsubscribe":
                    dashboard_stream.unsubscribe(client_id, msg.get("types", []))
        ...
        dashboard_stream.unregister_client(client_id)
    except Exception as e:
